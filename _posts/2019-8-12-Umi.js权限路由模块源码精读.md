...
Umi是阿里系推出的react应用框架，用于解决react企业级开发问题。Umi也是Ant Design Pro选用的几大重要组件之一。在写下这篇博文时，已经有5.6k赞。

Umi是已路由为基础的组件，在路由上延伸出了各种功能。所以研究路由模块对理解Umi真的非常重要。

更多Umi相关知识请看官网：https://umijs.org/zh/

说句题外话，可以略过这一段，感觉react和Vue的生态还不一样，Vue一般都是用指定的一些全家桶产品，路由就钦定`Vue-Router`，状态管理就钦定`Vuex`；而React...完全就感觉是在玩积木，各种各有的库都有，文档有的全有的不全，随时都需要手撕源码看里面的实现才知道怎么用，真叫一个渐进式...
...
也就是当routes有值时才渲染，渲染出来的结构是以，也很好理解。这个routes就对应路由配置文件中的routes数组（甚至名字都一样）。那么我们直接看routes有值时的情况吧。

Umi的设计的路由配置是**一个路由对应一个组件**，这样对于大型项目来说路由更加的清晰。React Router本来的设计是一个路由可以匹配到的所有组件都进行渲染，非常的灵活，除非用了`<Switch>`，才会只渲染第一个匹配到的组件。所以，我们也可以看到`renderRoutes`方法中返回的根组件是`<Switch>`。
...
也就是遍历路由数组中所有的配置`route`对象，每一个都根据实际情况渲染出一个路由组件`RouteRoute`（这起名风格我也是醉了，不看代码上下文完全没法猜意思）。
...
直接检查Routes属性，如果有，说明是权限路由，用一个另外的方法`withRoutes`进行响应，如果没有，用`RouteWithProps`进行响应。
